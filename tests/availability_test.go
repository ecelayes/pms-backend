...
func (s *AvailabilitySuite) TestGlobalAvailabilitySearch() {
	...
	for _, result := range response.Data {
		if result.UnitTypeID == s.unitTypeID {
			found = true
			break
		}
	}
...
func (s *AvailabilitySuite) TestAvailabilityPagination() {
	resR2 := s.MakeRequest("POST", "/api/v1/unit-types", map[string]interface{}{
		"property_id":       s.propertyID,
		"name":           "Suite UnitType",
		"code":           "SUI",
		"total_quantity": 5,
	...
	unitTypeID2 := dataR2["unit_type_id"]
	...
	s.NotEqual(response.Data[0].UnitTypeID, response2.Data[0].UnitTypeID)
}

func (s *AvailabilitySuite) TestAvailabilitySoldOut() {
	resR := s.MakeRequest("POST", "/api/v1/unit-types", map[string]interface{}{
		"property_id":       s.propertyID,
		"name":           "Limited UnitType", "code": "LTD",
		"total_quantity": 1,
		"base_price":     100.0,
		"max_occupancy":  2, "max_adults": 2, "max_children": 0,
		"amenities":      []string{"wifi"},
	...
	for _, result := range response2.Data {
		if result.UnitTypeID == unitTypeID {
			found = true
			break
		}
	}
...
func (s *AvailabilitySuite) TestAvailabilityMissingPrice() {
	resR := s.MakeRequest("POST", "/api/v1/unit-types", map[string]interface{}{
		"property_id":       s.propertyID,
		"name":           "No Price UnitType", "code": "NPR",
		"total_quantity": 5,
		"base_price":     0.0,
		"max_occupancy":  2, "max_adults": 2, "max_children": 0,
		"amenities":      []string{"wifi"},
